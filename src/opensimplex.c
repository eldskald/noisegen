...
static void __generate_os() {
    ...
    // Chain mip textures are for minmax reduction. At the end of the
    // process, the last mip texture should be a single pixel containing the
    // max value in the noise pixel on R and the min value on G.
    for (int i = 0; i < mip_count; i++) {
    ...
    // Normalization step. With texture with raw noise values and their min
    // and max values, we can normalize them in a normal R8G8B8A8 texture,
    // as well as apply range, power and inverted properties.
    __set_shader_property(
...
static void __resize_os() {
    ...
    SetTextureFilter(prop.texture, TEXTURE_FILTER_POINT);
